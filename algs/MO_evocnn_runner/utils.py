...
import numpy as np
import time
from algs.MO_evocnn_runner.genetic.population import Population, Individual, ConvUnit, FcUnit, PoolUnit
...
class Utils(object):
    ...
    @classmethod
    def load_train_log(cls, name):
        file_name = '%s/%s.txt' % (os.path.join(get_algo_local_dir(), 'log'), name)
        file_name = cls.path_replace(file_name)
        lines = []
        if os.path.exists(file_name):
            f = open(file_name, 'r')
