...
class Individual(object):
    def __init__(self, params, indi_no):
        ...
        self.timecost = 999999.0
        self.paramnum = 9999999999 # 10^10 -1
    ...
    def initialize(self):
        ...
        for t in type_arr:
            ...
            if 1 == t and 1 == flag:
                ...
                temp_img_size = int(img_size / pool.kernel_size[0])
                if temp_img_size < 1:
                    flag = 0
                    continue
                self.units.append(pool)
                img_size = temp_img_size   # default kernel_size[0] = kernel_size[1]
    ...
    def update_all_channels(cls, indi_units, type, log):
        ...
        shrink = 1
        # 程序会改变output_channel 程序是用来调成input_channel, 和之前的对应上
        for i in range(len(indi_units)):
            if indi_units[i].type == 1: #CONV

                # 修复第一个为空的情况 输入为input_channel
                if indi_units[i].in_channel is None:
                    indi_units[i].in_channel = input_channel
                    log.info('Due to the input-Conv, unit at %d changes its input channel from None to %d' % (
                        i, input_channel))
                else:
                    # generate log
                    if type == 0:
