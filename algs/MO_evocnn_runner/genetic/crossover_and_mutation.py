...
import numpy as np
import copy
from algs.MO_evocnn_runner.genetic.population import Individual
...
class Crossover(object):
    ...
    def _choose_two_parents(self, wh):
        ...
        idx1 = random.randint(0, max(self.T - 1, 2)) # 0-> 邻居个数 .. 在这里就限制了邻居范围
        idx2 = random.randint(0, max(self.T - 1, 2))
        while idx2 == idx1:
            idx2 = random.randint(0, max(self.T - 1, 2))
        """
        被选择的indi.  B[wh] 的邻居向量排序数组里的.. 第N个.. 那么需要将邻居向量对应到个体上
        """
