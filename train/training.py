...
class TrainModel(object):
    def __init__(self, file_id, logger): #file_id -> fileUri
        ...
        self.ptSavedPath = os.path.join("~/onebinary/ENASLogs/", algs_name ,"checkpoints/", work_name)
        if not os.path.exists(self.ptSavedPath):
            os.makedirs(self.ptSavedPath)
    ...
    def main(self, epoch):
        ...
        if test_loss / total < self.best_loss:
            ...
            #torch.save({'model': self.net.state_dict(), 'loss': self.best_loss}, "/root/demo123/MO-ResNet/checkpoints/pso_MNIST_complexity/" + self.file_id + ".pt")
            #torch.save({'model': self.net.state_dict(), 'loss': self.best_loss},"/root/demo123/MO-ResNet/checkpoints/test/" + self.file_id + ".pt")

            # 暂时不需要保存
            torch.save({'model': self.net.state_dict(), 'loss': self.best_loss}, os.path.join(self.ptSavedPath, self.file_id + ".pt"))
    ...
    def process(self):
        ...
        for p in range(total_epoch):
            ...
            self.train(p)
            self.main(p)
        return self.best_acc, self.best_error, 0.1 * self.best_loss, num_para # 0.1系数模拟一下实验
